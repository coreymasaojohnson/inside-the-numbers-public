<script lang="ts">
  ...
  // NEW: ID for managing concurrent async loads/renders to prevent race conditions
  let runId = 0; 
  ...
  async function loadDataForScope(s: Scope) {
    if (s === 'state') {
      ...
      console.log('[MapHero DEBUG] Full result keys:', Object.keys(fullResult));
      console.log('[MapHero DEBUG] Has breakdown?', 'breakdown' in fullResult);
      console.log('[MapHero DEBUG] Breakdown value:', fullResult.breakdown);
      ...
      // Additional debug: check if breakdown has data
      if (breakdownByFips.size === 0) {
        console.error('[MapHero ERROR] Breakdown data is empty! This means acs.ts has not been updated.');
        console.error('[MapHero ERROR] You need to apply the changes in acs-updates.ts to your acs.ts file.');
      } else {
        // Log a sample breakdown entry
        const sampleKey = Array.from(breakdownByFips.keys())[0];
        console.log('[MapHero DEBUG] Sample breakdown entry:', sampleKey, breakdownByFips.get(sampleKey));
      }
    }
  }

  // NEW: Get the appropriate value based on category
  function getValueForCategory(fips: CountyFips): number | undefined {
    ...
    if (!breakdown) {
      ...
      if (fips === '06037') { // LA County as example
        console.log('[MapHero DEBUG] No breakdown for', fips, 'Total breakdown size:', breakdownByFips.size);
      }
      return undefined;
    }
  ...
  function render() {
    ...
    // 🌟 FIX 1: Lock the container height to prevent it from collapsing when the old SVG is removed.
    if (svg) {
      container.style.height = `${height}px`; 
    }

    // If an SVG exists, fade it out before drawing fresh
    if (svg) {
      if (!reduceMotion) {
        svg
          .transition()
    ...
    // No existing SVG -> draw immediately
    drawFresh();
    ...
    function drawFresh() {
      ...
      // legend
      drawLegend(svg, width, height, projection);
      ...
      if (!reduceMotion) {
        svg
          ...
          .duration(180)
          .style('opacity', 1)
          // 🌟 FIX 2: Clear the fixed height on the container once the transition is complete.
          .on('end', () => {
             container.style.height = ''; 
          });
      } else {
        svg.style.opacity = 1;
        // 🌟 FIX 2 (Reduced Motion): Clear the fixed height immediately.
        container.style.height = ''; 
      }
    }
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      // Use runId for the initial load too
