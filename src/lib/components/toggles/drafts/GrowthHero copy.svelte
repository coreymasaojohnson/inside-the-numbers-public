<script lang="ts">
  ...
  // Updated to use new PUMS data loader
  ...
  const HEIGHT = 700; // Increased by ~27% from 550
  const margin = { top: 10, right: 130, bottom: 35, left: 50 };
  ...
  export let measure: 'indexed' | 'count' = 'count'; // Now controlled by parent
  ...
  async function fetchData() {
    ...
    try {
      ...
      console.log('Loaded time series data:', {
        dataPoints: dataTimeSeries.length,
        asianGroups: ASIAN_GROUPS.length,
        nhpiGroups: NHPI_GROUPS.length,
        years: YEARS
      });
    } catch (e) {
  ...
  function renderChart() {
    ...
    // Determine chart type based on measure
    const isStackedArea = measure === 'count'; // All views use stacked area for absolute counts
    const isIndexed = measure === 'indexed';

    const width = Math.max(320, cardContainer.clientWidth);
    ...
    if (isStackedArea) {
      // Stacked area for absolute counts (all views)
      // Filter stack data based on current view
      let keysToStack = ALL_STACK_KEYS;
      let stackDataFiltered = stackData;
      
      if (view === 'asian') {
        keysToStack = ASIAN_GROUPS;
        stackDataFiltered = stackData.map(row => {
          const filtered: any = { year: row.year };
          ASIAN_GROUPS.forEach(k => { filtered[k] = row[k] || 0; });
          return filtered;
        });
      } else if (view === 'nhpi') {
        keysToStack = NHPI_GROUPS;
        stackDataFiltered = stackData.map(row => {
          const filtered: any = { year: row.year };
          NHPI_GROUPS.forEach(k => { filtered[k] = row[k] || 0; });
          return filtered;
        });
      }
      
      const stackForView = d3
        .stack<any>()
        .keys(keysToStack)
        .order(d3.stackOrderNone)
        .offset(d3.stackOffsetNone);
      
      const stackedLayers = stackForView(stackDataFiltered);
    ...
    if (isStackedArea) {
      ...
      // Determine which keys and layers to use
      let keysToStack = ALL_STACK_KEYS;
      let stackDataFiltered = stackData;
      ...
      // Legend
      const legendData = keysToStack.map(k => ({ key: k, color: colorScale(k) as string })).reverse();
      ...
      group
        ...
        .text(d => {
          ...
          const formatted =
            measure === 'indexed' ? d3.format('.1f')(val) + '%' : formatNumber(val, { notation: 'compact' });
          return `${d.name} (${formatted})`;
        });
    }

    // Axes - Updated to show only the 3 time points
    g.append('g')
    ...
    const yLabel =
      isStackedArea 
        ? (view === 'combined' ? 'Total AANHPI Population' : 
           view === 'asian' ? 'Total Asian Population' : 'Total NHPI Population')
        : measure === 'count' ? 'Subgroup Population' : `Indexed Growth (${BASELINE_YEAR}=100)`;
    ...
    overlay
      ...
      .on('mousemove', (event: MouseEvent) => {
        ...
        if (isStackedArea) {
          ...
          let keysToShow = ALL_STACK_KEYS;
          if (view === 'asian') keysToShow = ASIAN_GROUPS;
          if (view === 'nhpi') keysToShow = NHPI_GROUPS;
          ...
          tooltipData = [{ name: 'Total Population', value: total, color: PALETTE.gray[6], isAbsolute: true }, ...arr];
        } else {
...
<div bind:this={cardContainer} class="bg-white relative font-[Inter]">
  ...
  <!-- Header & controls -->
  <div class="px-6 pt-4 pb-2" style="--scope-accent: {PALETTE.plum[7]}; --scope-accent-hover: {PALETTE.plum[3]}; --scope-accent-bg: {PALETTE.plum[0]};">
