<script lang="ts">
  ...
  $: if (initialized) { view; renderChart(); }
  $: if (initialized) { measure; renderChart(); }
  $: if (initialized) { YEARS; renderChart(); }
  $: if (initialized) { dataTimeSeries; renderChart(); }
  ...
  function drawChartContents(
    ...
    if (isStackedArea) {
      ...
      if (view === 'combined') {
        yMax = Math.max(yMax * 1.10, 13000000); // Add 10% padding or minimum 13M
      }
          ...
          if (lastPoint) {
            ...
            if (value >= LABEL_THRESHOLD) {
              ...
              g.append('text')
                ...
                .text(`${layer.key} (${formatNumber(value, { decimals: 0, notation: 'compact' })})`);
            } else {
        ...
        if (smallGroups.length > 0) {
          ...
          smallGroups.forEach((grp, i) => {
            ...
            legend.append('text')
              ...
              .style('fill', '#374151')
              .text(`${grp.key} (${formatNumber(grp.value, { decimals: 0, notation: 'compact' })})`);
          });
        }
      }
    } else {
    ...
    const yLabel = isStackedArea
      ? (view === 'combined' ? 'Disaggregated Population Count' :
         view === 'asian' ? 'Asian Subgroup Count' : 'NHPI Subgroup Count')
    ...
    overlay
      ...
      .on('mousemove', (event: MouseEvent) => {
        ...
        if (isStackedArea) {
          ...
          let total = 0;
          const keysForTooltip = view === 'asian' ? ASIAN_GROUPS : view === 'nhpi' ? NHPI_GROUPS : ALL_STACK_KEYS;
...
<div bind:this={cardContainer} class="bg-white relative font-[Inter]">
  ...
  <div class="mb-2">
    <span class="header-label">
      Data Period: {BASELINE_YEAR}–{YEARS[YEARS.length - 1] ?? '…'} (ACS 5-year estimates using 5-year intervals)
    </span>
  </div>
