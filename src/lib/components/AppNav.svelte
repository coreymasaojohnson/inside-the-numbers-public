					...
					{#each items as it, i}
						<li>
							<button
								...
								class:has-tooltip={it.id !== 'pop'}
								aria-current={active === it.id ? 'page' : undefined}
								aria-label={it.label}
								on:click={() => onNav(it.id)}
								data-tooltip={it.id === 'pop' ? '' : 'Coming soon'}
...
<style>
	...
	.appnav__link {
		...
		position: relative;
		padding-left: calc(0.375rem + var(--chev-space) + var(--chev-gap));
	}
	.appnav__link::before {
		content: var(--chev);
		position: absolute;
	...
	.appnav__link.has-tooltip::after {
	...
	.appnav__link.has-tooltip::before {
		content: '';
		position: absolute;
		bottom: calc(100% + 2px); /* Just below the tooltip */
		left: 50%;
		transform: translateX(-50%);
	...
	.appnav__link.has-tooltip:hover::after,
	.appnav__link.has-tooltip:hover::before {
		opacity: 1;
	}
	...
	.appnav .appnav__link.is-active {
		color: var(--plum-6);
	}
	.appnav .appnav__link:hover,
	.appnav .appnav__link.is-active:hover {
		color: var(--plum-5);
	}
