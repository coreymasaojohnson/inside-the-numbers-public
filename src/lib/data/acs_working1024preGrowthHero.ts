// src/lib/data/acs.ts
// 💡 UPDATED: Includes full implementation for time series subgroup composition.
...
const DEFAULT_GROWTH_YEARS = Array.from({ length: 13 }, (_, i) => 2010 + i);
...
async function fetchSubgroupsForYear(
  ...
  // Use ACS 1-year for time series
  const currentACSBase = `https://api.census.gov/data/${year}/acs/acs1`; 
  
  const totalSubgroups = tableId === 'B02018' ? 26 : 12; 
  ...
  try {
    ...
    const rawGroups: SubgroupRow[] = varCodes.map((code) => ({
      ...
      value: Number(dataRow[idx[code]]) || 0,
    }));
