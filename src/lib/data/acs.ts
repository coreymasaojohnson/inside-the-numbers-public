...
async function fetchSelectedGroupTotalsAnyCombo(
	groupId: 'B02018' | 'B02019'
): Promise<TreemapRow[]> {
...
export async function getAanhpiSubgroupsCombined_US(): Promise<TreemapRow[]> {
	if (TREEMAP_MEMO.has(TREEMAP_COMBINED_KEY as any))
		return TREEMAP_MEMO.get(TREEMAP_COMBINED_KEY as any)!;
...
async function fetchSubgroupsForYear(
	...
	if (year === 2020) {
		console.warn(
			`Skipping composition data for ${tableId} in 2020 as 1-year data is unavailable/unreliable.`
		);
		return null;
	}
	...
	if (year >= 2020) {
		// <-- CHANGED from 2018 to 2020 to stabilize older years
...
export async function getAanhpiSubgroupsCombinedTimeSeries(opts: {
	...
	const yearPromises = years.map(async (year) => {
		try {
			...
			if (asianResult && nhpiResult) {
				...
				const combinedGroups: SubgroupRow[] = [...asianResult.groups, ...nhpiResult.groups];
	...
	const results = (await Promise.all(yearPromises)).filter(
		(r): r is SubgroupSeriesRow => r !== null
	);
	...
	const asianGroups = latestGroups
		.slice(0, simplerSplitIndex > -1 ? simplerSplitIndex : latestGroups.length)
		.map((g) => g.name);
	const nhpiGroups = latestGroups
		.slice(simplerSplitIndex > -1 ? simplerSplitIndex : latestGroups.length)
		.map((g) => g.name);
...
export async function getAsianSubgroupsTimeSeries(opts: {
	...
	const promises = years.map((year) =>
		fetchSubgroupsForYear(year, 'B02018', opts.topN, opts.otherLabel)
	);
...
export async function getNhpiSubgroupsTimeSeries(opts: {
	...
	const promises = years.map((year) =>
		fetchSubgroupsForYear(year, 'B02019', opts.topN, opts.otherLabel)
	);
	const results = (await Promise.all(promises)).filter((r): r is SubgroupSeriesRow => r !== null);
...
type AapiCacheEntry = {
	...
	breakdown?: [string, { asian: number; nhpi: number }][];
	names: [string, string][];
	denom?: [string, number][];
};
...
export async function fetchAanhpiForeignBornByCountyDefensive(options?: {
	...
	const idx: Record<string, number> = Object.fromEntries(
		header.map((h: string, i: number) => [h, i])
	);
...
export async function getAanhpiForeignBornCached(options?: {
	...
	breakdown: Map<string, { asian: number; nhpi: number }>;
	...
	if (mem && Date.now() - mem.ts <= ONE_DAY) {
		return {
			...
			breakdown: mem.breakdown ? new Map(mem.breakdown) : new Map(),
	...
	if (ls) {
		...
		return {
			...
			breakdown: ls.breakdown ? new Map(ls.breakdown) : new Map(), // ← ADD THIS LINE
	...
	const entry: AapiCacheEntry = {
		...
		breakdown: Array.from(breakdown.entries()), // ← ADD THIS LINE
...
export async function fetchAanhpiForeignBornByState(options?: { withDenominator?: boolean }) {
	...
	const breakdown = new Map<string, { asian: number; nhpi: number }>(); // 💡 NEW: Initialize breakdown map
	...
	for (const r of data) {
		...
		breakdown.set(st, { asian, nhpi }); // 💡 FIX: Populate breakdown map

		if (denom) denom.set(st, +r[idx['B05002_013E']] || 0);
	}

	// 💡 FIX: Return breakdown map
	return { byState, breakdown, names, denomForeignBorn: denom };
}

export async function getAanhpiForeignBornByStateCached(withDenominator = true) {
	const key = `aanhpiFB:2022:STATE:v2:${withDenominator ? 'den' : 'noden'}`;

	const mem = MEMO_STATE.get(key);
	if (mem && Date.now() - mem.ts <= ONE_DAY) {
		return {
			byState: new Map(mem.byFips),
			breakdown: mem.breakdown ? new Map(mem.breakdown) : new Map(), // ✅ Added for memory cache
			names: new Map(mem.names),
			denomForeignBorn: mem.denom ? new Map(mem.denom) : undefined
		};
	}
	...
	if (ls) {
		...
		if (Date.now() - parsed.ts <= ONE_DAY) {
			...
			return {
				...
				breakdown: parsed.breakdown ? new Map(parsed.breakdown) : new Map(), // ✅ Added for local storage cache
	...
	// ✅ Destructure 'breakdown' from the fetch function
	const { byState, names, denomForeignBorn, breakdown } = await fetchAanhpiForeignBornByState({
		withDenominator
	});
	...
	const entry: AapiCacheEntry = {
		...
		names: Array.from(names.entries()),
		breakdown: Array.from(breakdown.entries()), // ✅ Stored in cache entry
		denom: denomForeignBorn ? Array.from(denomForeignBorn.entries()) : undefined
	};
	...
	// ✅ Return breakdown for a cache miss
	return { byState, names, denomForeignBorn, breakdown };
}
