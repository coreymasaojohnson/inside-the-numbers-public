...
export async function getAanhpiForeignBornCached(options?: {
  ...
  // 3) Fetch + cache
  const { byFips, breakdown, names, denomForeignBorn } =  // ← ADD breakdown HERE
...
export async function getAanhpiForeignBornByStateCached(withDenominator = true) {
  ...
  const { byState, names, denomForeignBorn, breakdown } = await fetchAanhpiForeignBornByState({ withDenominator });
