...
import {
  ASIAN_CANONICAL_RULES, ASIAN_COMPUTED_OTHER,
  NHPI_CANONICAL_RULES,  NHPI_COMPUTED_OTHER,
...
async function getJSON<T>(url: string): Promise<T> {
  ...
  const final = needsKey && CENSUS_API_KEY
...
// ── Data fetch + shaping ──────────────────────────────────────────────────
async function fetchUSValues(year: number, codes: string[], acsType: AcsType): Promise<Record<string, number>> {
...
async function getSubgroupsCrosswalked_US(
  ...
  const { entries, totalCode } = buildCrosswalkForYear(year, groupId, meta, rules, computedOtherLabel);
  ...
  const includedForOther = entries.filter(e => !e.excludeFromOther && e.canonical !== computedOtherLabel);
  const includedSum = includedForOther.reduce((s, e) => s + e.codes.reduce((t, c) => t + (values[c] || 0), 0), 0);
...
export async function getAsianSubgroups_US(year = 2022, acsType: AcsType = 'acs5'): Promise<TreemapRow[]> {
...
export async function getNhpiSubgroups_US(year = 2022, acsType: AcsType = 'acs5'): Promise<TreemapRow[]> {
  return getSubgroupsCrosswalked_US(
    year,
...
export async function getAanhpiSubgroupsCombined_US(year = 2022, acsType: AcsType = 'acs5'): Promise<TreemapRow[]> {
...
export async function getSubgroupCrosswalk(year: number, acsType: AcsType = 'acs5') {
  ...
  const cwA = buildCrosswalkForYear(year, ACS_SUBGROUPS_CONFIG.asian.tableId, metaA, ASIAN_CANONICAL_RULES, ASIAN_COMPUTED_OTHER);
  const cwN = buildCrosswalkForYear(year, ACS_SUBGROUPS_CONFIG.nhpi.tableId, metaN, NHPI_CANONICAL_RULES, NHPI_COMPUTED_OTHER);

  return {
    year,
    asian: cwA.entries.map(e => ({ canonical: e.canonical, codes: e.codes, excludeFromOther: e.excludeFromOther })),
    nhpi:  cwN.entries.map(e => ({ canonical: e.canonical, codes: e.codes, excludeFromOther: e.excludeFromOther }))
  };
}
