<script lang="ts">
  ...
  $: introScale = $isGrowth 
    ? ($growthScale === 'absolute' ? 'Absolute' : 'Indexed')
    : '';
...
<main class="site-container site-container--flush mt-2.5">
  <HeroPanel isOpen={true} onToggle={() => {}}>
    ...
    <div 
      class="hero-load-wrapper"
      class:loaded={heroLoadComplete}
    >
      {#key $currentView}
        <div
        ...
        >
          ...
          {:else if $currentView === 'Treemap View'}
            <TreemapHero
              family={$treemapFamily} palette="plum"
              showHeader={true}
              showLegend={true}
              onFirstRenderComplete={handleHeroRenderComplete}
            />
...
<div 
  class="intro-load-wrapper"
  class:loaded={introLoadComplete}
>
  <IntroBelow 
    activeView={introView}
    activeScope={introScope}
    activeScale={introScale}
  />
</div>
