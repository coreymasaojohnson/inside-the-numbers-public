...
function inferFamily(basename) {
  ...
  const stopIndex = parts.findIndex(p =>
    /(thin|ultra|extra|light|book|regular|roman|text|medium|semi|demi|bold|heavy|black|italic|oblique|vf|variable)/i.test(p)
  );
...
for (const fam in byFamily) {
  byFamily[fam].sort((a,b) => (a.variable === b.variable ? a.weight - b.weight : a.variable ? -1 : 1));
}
